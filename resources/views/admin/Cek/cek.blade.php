...
@section('content')
    <section class="section">
        ...
        <div class="section-body">
            <div class="pb-4">
                <!-- <a href="{{ route('admin.cek.tambah') }}" class="btn btn-primary" type="button">
                    Tambah Data
                </a> -->
                <a href="{{ route('admin.cek.tambah') }}" class="btn btn-primary" type="button">
                    Cek Kepribadian
                </a>
                <a href="{{ route('cek.pdf') }}" target="_blank" class="btn btn-warning text-dark" type="button">
                    Cetak Data
                </a>
            </div>
            @if (session('success'))
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-striped" id="table-1">
                            ...
                            @endphp
                                @foreach ($ceks as $cek => $value)
                                    <tr>
                                        <td class="text-center">
                                            {{ $counter }}
                                        </td>
                                        <td>{{ $value['kepribadian']['name'] }}</td>
                                        <td>{{ $value['indikasi']['name'] }}</td>
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                    <a class="dropdown-item btn btn-outline-warning"
                                                        href="{{ route('admin.cek.edit', ['id' => $value['id']]) }}">Edit</a>
                                                    <a class="dropdown-item btn btn-outline-danger" id="btnHapus">Hapus</a>
                                                    <form id="formHapus"
                                                        action="{{ route('admin.cek.destroy', ['id' => $value['id']]) }}"
                                                        method="post">
                                                        @csrf
                                                        @method('delete')
                                                    </form>
                                    ...
                                    @php
                                    $counter++; // Inisialisasi variabel counter
                                    @endphp
                                @endforeach
                            </tbody>
                        </table>
