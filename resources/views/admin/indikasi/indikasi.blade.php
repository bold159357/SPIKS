...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>Halaman Indikasi</h1>
        </div>
        <div class="section-body">
            <div class="pb-4">
                <a href="{{ route('admin.indikasi.tambah') }}" class="btn btn-primary" type="button">
                    Tambah Data
                </a>
                <a href="{{ route('indikasi.pdf') }}" target="_blank" class="btn btn-warning text-dark" type="button">
                    Cetak Data
                </a>
            </div>
            @if (session('success'))
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                    <a class="dropdown-item btn btn-outline-warning"
                                                        href="{{ route('admin.indikasi.edit', ['id' => $p->id]) }}">Edit</a>
                                                    <a class="dropdown-item btn btn-outline-danger" id="btnHapus">Hapus</a>
                                                    <form id="formHapus"
                                                        action="{{ route('admin.indikasi.destroy', ['id' => $p->id]) }}"
                                                        method="post">
                                                        @csrf
                                                        @method('delete')
                                                    </form>
